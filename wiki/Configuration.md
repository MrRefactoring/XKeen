...
Штатная балансировка Xray выбирает ноду по задержке (`leastPing`). Флаг `xkeen -sb` добавляет выбор по **фактической скорости**: по расписанию замеряет каждую ноду балансировщика и переставляет выбор на самую быструю, если она обгоняет текущую больше чем на порог переключения (гистерезис). Механизм внешний — он не переписывает конфигурацию Xray и не перезапускает его, а управляет балансировщиком через gRPC api (`OverrideBalancerTarget`).
